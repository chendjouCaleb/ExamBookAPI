...
namespace ExamBookTest.Services
{
	public class ClassroomServiceTest
	{
        ...
        public async Task GetClassroomById()
        {
            ...
            var resultClassroom = await _classroomService.GetByIdAsync(classroom.Id);
            Assert.AreEqual(classroom.Id, resultClassroom.Id);
        }


        [Test]
        public void GetNonExistingClassroom_ShouldThrow()
        {
            var badId = ulong.MaxValue;
            var ex = Assert.ThrowsAsync<ElementNotFoundException>(async () =>
            {
                await _classroomService.GetByIdAsync(badId);
            });

            Assert.AreEqual("ClassroomNotFoundById", ex!.Code);
            Assert.AreEqual(badId, ex.Params[0]);
        }
        ...
        public void GetNonExistingClassroomByName_ShouldThrow()
        {
            ...
            var ex = Assert.ThrowsAsync<ElementNotFoundException>(async () =>
            {
                await _classroomService.GetByNameAsync(_space, badName);
            });

            Assert.AreEqual("ClassroomNotFoundByName", ex!.Message);
            Assert.AreEqual(badName, ex.Params[0]);
            Assert.AreEqual(_space, ex.Params[1]);
        }
        ...
        public async Task AddClassroom()
        {
            ...
            await assertions.HasPublisherIdAsync(classroom.PublisherId);
        }
        ...
        public async Task SetClassroomRoom()
        {
            ...
            var newRoom = (await _roomService.AddAsync(_space, 
        ...
        public async Task ChangeClassroomRoom()
        {
            ...
            Publisher roomPublisher = await _publisherService.GetByIdAsync(_room.PublisherId);
            Publisher newRoomPublisher = await _publisherService.GetByIdAsync(newRoom.PublisherId);
            Publisher publisher = await _publisherService.GetByIdAsync(classroom.PublisherId);
            Publisher spacePublisher = await _publisherService.GetByIdAsync(_space.PublisherId);
